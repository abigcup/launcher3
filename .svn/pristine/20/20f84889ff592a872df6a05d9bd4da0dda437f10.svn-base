 ...
 * </pre>
 */
public final class ShellUtils2 {
    ...
    private static String getRootCommand(boolean isRooted){
        if(!isRooted)
            return "sh";

        if(isFileExists("/system/bin/cph_su")){
            return "cph_su";
        } else if (isFileExists("/system/xbin/xu")) {
            return "xu";
        } else {
            return "su";
        }
    ...
    public static CommandResult execCmd(final String[] commands,
        ...
        try {
            process = Runtime.getRuntime().exec(getRootCommand(isRooted));
