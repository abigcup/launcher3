...
public class InstallShortcutReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent data) {
        ...
        for (int i = 0; i < LauncherModel.sBgWorkspaceItems.size(); ++i) {
            //如果是和预置下载图标名字重复也继续执行而不是返回
            //快捷方式允许重名(activityInfo!=null说明是应用，不是快捷方式)
            if (name.equals(LauncherModel.sBgWorkspaceItems.get(i).title)
                    && info.activityInfo != null
                    && LauncherModel.sBgWorkspaceItems.get(i).itemType != LauncherSettings.Favorites.ITEM_TYPE_PRESET_SHORTCUT) {
                Log.e(TAG, "PresetInfo return:" + name);
                return;
            }
        }
    ...
    private static void queuePendingShortcutInfo(PendingInstallShortcutInfo info, Context context) {
        ...
        Log.i(TAG, "queuePendingShortcutInfo getPackageName:" + packageName);
        if (LauncherModel.isIgnoreShortcutApp(packageName)) {
            return;
        }
    ...
    private static PendingInstallShortcutInfo convertToLauncherActivityIfPossible(
        ...
        if (original.isLuncherActivity()) {
            ...
            Log.e(TAG, "Already an activity target");
            return original;
        }
        if (!Utilities.isLauncherAppTarget(original.launchIntent)
            ...
            Log.e(TAG, "We can only convert shortcuts which point to a main activity in the current user.");
            return original;
        }
