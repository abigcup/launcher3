...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected void notifyPageSwitchListener() {
        if (mPageSwitchListener != null) {
            System.out.println("cym  notifyPageSwitchListener "+getNextPage()+"  "+new AppUsageUtil().checkPermission(getContext()));
            if(getNextPage()==0 &&
                    ! new AppUsageUtil().checkPermission(getContext())) {

                Intent intent = new Intent(getContext(), FirstScreenAcessActivity.class);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
         ...
         * scrolling there.
         */
        System.out.println("cym  ******* onInterceptTouchEvent "+mCurrentPage );
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                System.out.println("cym  ******* MotionEvent.ACTION_DOWN "+mCurrentPage );
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        System.out.println("cym  ******* onTouchEvent "+mCurrentPage );
        ...
        case MotionEvent.ACTION_UP:
            System.out.println("cym  ******* mCurrentPage222 "+mCurrentPage );
            if (true) {
                ...
                System.out.println("cym  ******* mCurrentPage111 "+mCurrentPage );
                if (!mFreeScroll) {
                    ...
                    Log.d("cym ismScreenIsLoop",SettingSharedPreferencesManager.getInstance().ismScreenIsLoop(getContext())+"");
                    //是否循环滑动
                    if (SettingSharedPreferencesManager.getInstance().ismScreenIsLoop(getContext())) {
                        System.out.println("cym  ******* mCurrentPage "+mCurrentPage +returnToOriginalPage);
                        if (((isSignificantMove && deltaX > 0 && !isFling) ||
                                (isFling && velocityX > 0)) && mCurrentPage >= 0) {
                            System.out.println("cym  ******* mCurrentPage left");
                            if (mCurrentPage == 0) {
                                finalPage = returnToOriginalPage ? mCurrentPage : getChildCount() - 1;
                            } else {
                                finalPage = returnToOriginalPage ? mCurrentPage : mCurrentPage - 1;
                            }
                            if(mCurrentPage == 0 && !returnToOriginalPage){
                            ...
                            System.out.println("cym  ******* mCurrentPage right");
                    ...
                    if(DEBUG) {
                        Log.d(TAG, "mCurrentPage=" + mCurrentPage +
                                ",returnToOriginalPage=" + returnToOriginalPage +
                                ",finalPage=" + finalPage +
                                ",isSignificantMove=" + isSignificantMove +
                                ",x="+x+",mDownMotionX="+mDownMotionX+",deltaX="+deltaX+",velocityX="+velocityX+
                                ",isDeltaXLeft=" + isDeltaXLeft + ",isVelocityXLeft=" + isVelocityXLeft);
                    }
                } else {
                    if (!mScroller.isFinished()) {
                        abortScrollerAnimation(true);
