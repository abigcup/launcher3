...
public class FolderIcon extends FrameLayout implements FolderListener {
    ...
    private boolean willAcceptItem(ItemInfo item) {
        ...
        return ((itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION
                ...
                || itemType == LauncherSettings.Favorites.ITEM_TYPE_PRESET_SHORTCUT
    ...
    public void onDrop(DragObject d) {
        ...
        if (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_PRESET_SHORTCUT) {
            synchronized (LauncherModel.sBgLock) {
                if (LauncherModel.getItemFromSbItemsByPackageName(item.getIntent().getPackage()) == null) {
                    return;
                }
            }
        }
    ...
    private PreviewItemDrawingParams computePreviewItemDrawingParamsEx(int index,
                                                                       PreviewItemDrawingParams params, int iconSize) {
//        index = NUM_ITEMS_IN_PREVIEW - index - 1;
        float r = (index * 1.0f) / (NUM_ITEMS_IN_PREVIEW - 1);
        float scale = (1 - PERSPECTIVE_SCALE_FACTOR * (1 - r));
        ...
        if (params == null) {
            ...
            // TODO: 2018/1/23
            //这段逻辑用了一些常量，不太准确。（和我想的不太一样。。。）
            //84是目录图标实际宽高（去掉透明色的像素）
            int folderIcoRealLength = grid.iconSizePx + 2;
            int leftOrTopOffset = 5; // 3x3排列头尾的间隔
            int itemSpace = 5; // 3x3排列中间的间隔
            int xPos = (getWidth() - folderIcoRealLength) / 2 + leftOrTopOffset;
            int yPos = (getHeight() - folderIcoRealLength) / 2 + grid.folderBackgroundOffset + leftOrTopOffset;
            int row = index / 3;
            int col = index % 3;

            //每个小图标的
            int length = (folderIcoRealLength - leftOrTopOffset * 2 - itemSpace * 2) / 3;
            params.transX = col * (length + itemSpace) + xPos;
            params.transY = row * (length + itemSpace) + yPos;
            params.scale = (float) length / iconSize;
            params.overlayAlpha = overlayAlpha;
        }
        return params;
    }
        ...
        if (!mAnimating) {
            for (int i = nItemsInPreview - 1; i >= 0; i--) {
                ...
                if (!mHiddenItems.contains(v.getTag())) {
                    d = getTopDrawable(v);
                    computePreviewDrawingParams(d);
                    mParams = computePreviewItemDrawingParamsEx(i, mParams, d.getIntrinsicWidth());
                    mParams.drawable = d;
