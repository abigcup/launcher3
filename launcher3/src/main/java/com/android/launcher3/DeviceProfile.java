...
public class DeviceProfile {
    ...
    private int hotseatBarMarginPx;
    ...
    public DeviceProfile(Context context, InvariantDeviceProfile inv,
        ...
        pageIndicatorHeightPx = Utilities.pxFromDp(inv.pageIndicatorHeight, dm);
//                res.getDimensionPixelSize(R.dimen.dynamic_grid_page_indicator_height);
        ...
        iconDrawablePaddingOriginalPx = Utilities.pxFromDp(inv.iconDrawablePadding, dm);
//                res.getDimensionPixelSize(R.dimen.dynamic_grid_icon_drawable_padding);
        hotseatBarMarginPx = Utilities.pxFromDp(inv.hotseatBarMargin, dm);//res.getDimensionPixelSize(R.dimen.dynamic_grid_hotseat_margin);
        keepSize = inv.keepIconSize;
    ...
    private void updateAvailableDimensions(DisplayMetrics dm, Resources res) {
        ...
        //TODO:横屏情况，还没有彻底调整好。目前只调整了竖屏情况 20181210 lijy
        //这里没有完全理解,不知道有没有改错了(不考虑头部，尾部的话，才会是全铺。所以这里扣除下必要的显示，然后换算下真正要显示的大小)
        if (isLandscape){
            float useHeight = (availableWidthPx - workspacePadding.top - workspacePadding.bottom);

            scale = useHeight / availableWidthPx;
        }else{
            float useHeight = (availableHeightPx - workspacePadding.top - workspacePadding.bottom);

            scale = useHeight / availableHeightPx;
        }
        ...
        if(scale>0) {
//            drawablePadding = 0;// 图标与文字间距，设置成可配置
    ...
    Rect getWorkspacePadding(boolean isLayoutRtl) {
        ...
        if (isLandscape && transposeLayoutWithOrientation) {
            ...
            if (isLayoutRtl) {
                padding.set(hotseatBarHeightPx + hotseatBarMarginPx, edgeMarginPx,
                        searchBarBounds.width(), edgeMarginPx);
            } else {
                padding.set(searchBarBounds.width(), edgeMarginPx,
                        hotseatBarHeightPx + hotseatBarMarginPx, edgeMarginPx);
            }
        } else {
            if (isTablet) {
                // Pad the left and right of the workspace to ensure consistent spacing
                ...
                int paddingBottom = hotseatBarHeightPx + pageIndicatorHeightPx + hotseatBarMarginPx;
                ...
                if (isLandscape){
                    padding.set(iconSizePx + edgeMarginPx,
                            edgeMarginPx,
                            iconSizePx +  edgeMarginPx,
                            hotseatBarHeightPx + pageIndicatorHeightPx + hotseatBarMarginPx);
                }else {
    ...
    public void layout(Launcher launcher) {
        ...
        if (hasVerticalBarLayout) {
            ...
            lp.width = LayoutParams.MATCH_PARENT;
            lp.height = hotseatBarHeightPx;
            lp.bottomMargin = hotseatBarMarginPx;
            if (isLandscape) {
                hotseat.findViewById(R.id.layout).setPadding(iconSizePx + edgeMarginPx, 0,
                        iconSizePx + edgeMarginPx, 0);
                hotseat.setBackground(launcher.getResources().getDrawable(R.drawable.bg_hotseat_land));
            }else{
                hotseat.findViewById(R.id.layout).setPadding(edgeMarginPx, 0,
                        edgeMarginPx, 0);
                hotseat.setBackground(launcher.getResources().getDrawable(R.drawable.bg_hotseat_port));
            }
        ...
        if (pageIndicator != null) {
            if (hasVerticalBarLayout) {
                ...
                lp.bottomMargin = hotseatBarHeightPx + pageIndicatorHeightPx/2 + hotseatBarMarginPx;
        ...
        if (overviewMode != null) {
            ...
            lp.width = (int) (availableWidthPx * 1);
